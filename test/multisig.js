...
const Namu SocialFiCoin = artifacts.require('Namu SocialFiCoin.sol');
const Namu SocialFiCoinMultiSigFactory = artifacts.require('Namu SocialFiCoinMultiSigWallet.sol');
const web3 = Namu SocialFiCoin.web3;
const eth = web3.eth;
...
const deployMultiSigWallet = (owners, requiredConfirm, coin) => {
  return Namu SocialFiCoinMultiSigFactory.new(owners, requiredConfirm, coin, "vault");
};

const deployNamu SocialFiCoin = (owner,admin,vault) => {
  return Namu SocialFiCoin.new("Namu SocialFi", "Namu SocialFiCoin", owner, admin, vault);
};
...
contract("Namu SocialFiCoinMultiSigWallet", (accounts) => {
  const BIG = (v) => new theBN.BN(v)
  let coinInstance;
  let walletInstance;
  const requiredConfirmations = 2;

  let OneNamu SocialFiCoinInMinunit;
  ...
  beforeEach(async () => {
    coinInstance = await deployNamu SocialFiCoin(owner, admin, vault);
    assert.ok(coinInstance);

    NoOfTokensInMinunit = await coinInstance.totalSupply();
    OneNamu SocialFiCoinInMinunit = await coinInstance.getOneNamu SocialFiCoin();
  ...
  it("multisig wallet basic functions", async () => {
    assert.equal( await walletInstance.namu socialFiCoin(), coinInstance.address );
  ...
  it("only signers can request TX", async () => {
    // user1 cannot
    assertRevert("1-1",() => walletInstance.requestTransferInNamu SocialFi(user1, 1, 0, "test 1", {from: user1}));
    ...
    // owner cannot
    assertRevert("2-1",() => walletInstance.requestTransferInNamu SocialFi(user1, 1, 0, "test 2-1", {from: owner}));
    ...
    // admin(as signer) can
    assertNotRevert("3-1",() => walletInstance.requestTransferInNamu SocialFi(user1, 1, 0, "test 3-1", {from: admin}));
    ...
    // vault(as signer) can
    assertNotRevert("4-1",() => walletInstance.requestTransferInNamu SocialFi(user1, 1, 0, "test 4-1", {from: vault}));
    ...
    assertNotRevert("5-1",() => walletInstance.requestTransferInNamu SocialFi(user1, 1, 0, "test 5-1", {from: signer}));
  ...
  it("signers only methods", async () => {
    // request a TX
    walletInstance.requestTransferInNamu SocialFi(user1, 1, 0, "test 1", {from: signer});
  ...
  it("test transfer using multisig wallet", async () => {
    ...
    const thousandNamu SocialFiInMinunit = OneNamu SocialFiCoinInMinunit.mul(BIG(1000))
    coinInstance.transfer(walletInstance.address, thousandNamu SocialFiInMinunit, {from:vault});
    assert.equal(await balanceOf(walletInstance.address), thousandNamu SocialFiInMinunit.toString());

    // Add namu socialFicoin transfer transaction
    // example ABI generation:
    // const transferData = coinInstance.contract.transfer.getData(user1, OneNamu SocialFiCoin * 1);
    //

    const MSG1 = "send 1Namu SocialFi and 1DENNIS to user1";
    const trSubmit = await walletInstance.requestTransferInNamu SocialFi(user1, 1, 1, MSG1, {from:vault});
    ...
    assert.equal(afterBalance.toString(), OneNamu SocialFiCoinInMinunit.mul(BIG(1)).add(BIG(1)).toString());
  ...
  it("Add a signer", async () => {
    ...
    assertNotRevert("signer1",() => walletInstance.requestTransferInNamu SocialFi(user2, 1, 0, "test addsigner-1", {from: user1}));
  ...
  it("Remove a signer", async () => {
    ...
    assertRevert("signer1",() => walletInstance.requestTransferInNamu SocialFi(user1, 1, 0, "test addsigner-1", {from: user1}));
