...
// File: contracts/NamuSocialFiCoin.sol
 ...
 * @title coin contract
 */
contract NamuSocialFiCoin is ERC20Capped, ERC20Detailed, ERC20Burnable {
...
// File: contracts/NamuSocialFiCoinSafeMath.sol
 ...
 */
library NamuSocialFiCoinSafeMath {
...
// File: contracts/NamuSocialFiCoinMultiSigWallet.sol
...
/**
 * @title NamuSocialFiCoin Multi Signature contract
 */
contract NamuSocialFiCoinMultiSigWallet {
    /*  Enums  */
    ...
    /* Contract Created */
    event MultiSigCreated(address indexed creator, address indexed NamuSocialFiCoin);
    ...
    using NamuSocialFiCoinSafeMath for uint8;
    using SafeMath for uint256;
    ...
    NamuSocialFiCoin public NamuSocialFiCoin;
     ...
     * @param _NamuSocialFiCoinAddress NamuSocialFiCoin address
     *
     */
    constructor (address[] memory _signers, uint8 _requiredSign, address _NamuSocialFiCoinAddress, string memory _description) public {
        ...
        nextTx = 0;
        NamuSocialFiCoin = NamuSocialFiCoin(_NamuSocialFiCoinAddress);
        owner = msg.sender;
        description = _description;
        emit MultiSigCreated(owner, _NamuSocialFiCoinAddress);
    }
    ...
    /**
      ...
      * @param _amountUnit amount of NamuSocialFiCoin(Play part) to be transferred
      * @param _amountMinunit amount of NamuSocialFiCoin(Minunit part) to be transferred
      */
    ...
    // NamuSocialFiCoin.transfer will be fail with reason out of gas. Then the TX will remain
    ...
    function runTX(uint16 txid) internal senderIsSigner returns(bool) {
        ...
        if(request.txType == TxType.TRANSFER) {
            // transfer money
            NamuSocialFiCoin.transfer(request.who, request.amount);
            ...
            NamuSocialFiCoin.setAdmin(request.who);
        } else if(request.txType == TxType.OWNER_CHANGE) {
            NamuSocialFiCoin.setOwner(request.who);
        } else if(request.txType == TxType.VAULT_CHANGE) {
            NamuSocialFiCoin.setVault(request.who);
        } else {
            return false;
        }
        return true;
